...
class Grid:
    ...
    def __init__(self, width, height, length, voxelSize, density, materialMap, numSources):
        ...
        self.scoring = np.zeros((self.xSize, self.ySize, self.zSize), dtype=float)     # keep track of deposited dose in a voxel by proton [MeV]
        ...
        self.SP_water = np.loadtxt("SPdata/SP_water.txt", 'float', '#', None, None, 8)
        self.SP_bone = np.loadtxt("SPdata/SP_bone.txt", 'float', '#', None, None, 8) # ADDED ONE FOR TESTING
        self.SP_database = []
    ...
    def compute_mean_tv_dose(self, voxelsCancer):
        ...
        dose = self.scoring / (self.density * voxel_volume) # (MeV / g)
        dose = dose * 1000 # (MeV / kg)
        dose = dose * 1.602176e-19 # (Gy = J/kg) # mean dose per proton
        dose = dose*500*.4e9 # total dose, adjusted for many particles 

        for l in range(0,len(voxelsCancer)):
            doseTV.append(dose[voxelsCancer[l][0], voxelsCancer[l][1], voxelsCancer[l][2]]) # for each cancer voxel, append the dose
    ...
    def irradiate(self, SIPP, SIPE, ES, MCS, PLOT, particlesNumber, sigmaX, sigmaY, sigmaE, Beam_position, Beam_direction, Beam_mean_energy, offset, step_length, center=None, rad=-1):
        ...
        dose = dose * 1000 # (MeV / kg)
        dose = dose * 1.602176e-19 # (Gy = J/kg) # mean dose per proton
        dose = dose*particlesNumber*.4e9 # total dose, adjusted for many particles 
        
        print(np.amax(dose))

        '''if center is None:
            self.compute_center()
            x, y, z = self.centerX, self.centerY, self.centerZ
        else:
            x, y, z = center''' 
    ...
    def compute_center(self):
        """Compute center of the tumors according to number of cancer of cells"""
        ...
        for k in range(self.zSize):
            for i in range(self.xSize):
                for j in range(self.ySize):
                    ...
                    sumZ += k * ccell_count
                    count += ccell_count
        self.centerX = sumX / count
        self.centerY = sumY / count
        self.centerZ = sumZ / count
